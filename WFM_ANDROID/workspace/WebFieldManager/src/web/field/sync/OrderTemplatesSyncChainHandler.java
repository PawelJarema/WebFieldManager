...
import web.field.model.json.JsonOrderTemplate;

public class OrderTemplatesSyncChainHandler implements ISyncChainHandler {

	private ISyncChainHandler nextHandler;

	public OrderTemplatesSyncChainHandler(ISyncChainHandler nextHandler) {
		...
		LongRunningHttpRequest getTemplates = new LongRunningHttpRequest(
			...
			protected void onPostExecute(String results) {
				if (results != null && !"false".equals(results)) {
					...
					List<JsonOrderTemplate> answer = gson.fromJson(results,
							new TypeToken<List<JsonOrderTemplate>>() {
							}.getType());

					db.loadOrderTemplates(answer);
