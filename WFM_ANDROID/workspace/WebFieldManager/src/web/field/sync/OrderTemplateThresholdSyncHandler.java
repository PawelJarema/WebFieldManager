...
import web.field.model.json.JsonOrderTemplateThreshold;

public class OrderTemplateThresholdSyncHandler implements ISyncChainHandler {

	private ISyncChainHandler nextHandler;

	public OrderTemplateThresholdSyncHandler(ISyncChainHandler nextHandler) {
	...
	public void handle(final SyncCommand command) {
		HttpPost templatesRequest = new HttpPost(command.getServiceAddress() + "GetOrdersTemplateThresholds/");
		...
		LongRunningHttpRequest getTemplates = new LongRunningHttpRequest(
			...
			protected void onPostExecute(String results) {
				if (results != null && !"false".equals(results)) {
					...
					List<JsonOrderTemplateThreshold> answer = gson.fromJson(results,
							new TypeToken<List<JsonOrderTemplateThreshold>>() {
							}.getType());

					db.loadOrderTemplateThresholds(answer);
